...
set_dunst_config() {
	sed -i "$HOME"/.config/bspwm/dunstrc \
		...
		-e "s/frame_color = .*/frame_color = \"#f5edd8\"/g" \
		-e "s/separator_color = .*/separator_color = \"#fe8019\"/g" \
		-e "s/font = .*/font = JetBrainsMono NF Medium 9/g" \
		-e "s/foreground='.*'/foreground='#3C3836'/g"
...
set_launcher_config () {
	sed -i "$HOME/.config/bspwm/scripts/Launcher.rasi" \
		...
		-e 's/\(background: \).*/\1#fefbef;/' \
		-e 's/\(background-alt: \).*/\1#f4f5f8E0;/' \
		-e 's/\(foreground: \).*/\1#3c3836;/' \
		-e 's/\(selected: \).*/\1#211B02;/' \
		-e 's/[^/]*-rofi/al-rofi/'
}
