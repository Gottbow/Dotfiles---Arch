...
set_jgmenu_colors() {
	sed -i "$HOME"/.config/bspwm/jgmenurc \
		-e 's/color_menu_bg = .*/color_menu_bg = #11111b/' \
		-e 's/color_norm_fg = .*/color_norm_fg = #bd93f9/' \
		-e 's/color_sel_bg = .*/color_sel_bg = #bd93f9/' \
		-e 's/color_sel_fg = .*/color_sel_fg = #ffffff/' \
		-e 's/color_sep_fg = .*/color_sep_fg = #f1fa8c/'
}
...
launch_bars() {
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		...
		MONITOR=$mon polybar -q cristina-bar2 -c ${rice_dir}/config.ini &
		MONITOR=$mon polybar -q cristina-bar3 -c ${rice_dir}/config.ini &
		MONITOR=$mon polybar -q cristina-bar4 -c ${rice_dir}/config.ini &
	done
